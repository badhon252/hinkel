...
const createCouponIntoDb = async (payload) => {
    const existing = await Coupon.findOne({ codeName: payload.codeName?.toUpperCase().trim() });
    if (existing) throw new AppError("Coupon with this code name already exists", 409);
...
const getCouponByCodeFromDb = async (codeName) => {
    const coupon = await Coupon.findOne({ codeName: codeName?.toUpperCase().trim() });
